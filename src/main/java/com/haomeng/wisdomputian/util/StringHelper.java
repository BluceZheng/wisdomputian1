...
public final class StringHelper {
	...
	public static String passwordBuilder(String passrule, int length) // 随机生成动态密码
	{
		...
		case 3:
			String[] tmp_arr3 = { "2", "3", "4", "5", "6", "7", "8", "9", "a", "d", "e", "f", "g", "h", "i", "j", "m",
					"n", "r", "t", "u", "y", "A", "B", "D", "E", "F", "G", "H", "J", "L", "M", "N", "Q", "R", "T",
					"Y" }; // 大写小写数字
			arr = tmp_arr3;
			break;
		case 4:
	 ...
	 * StringHelper.replaceRange("this is a test sentense,Look at
	 * this!","test","look ","stone!",true) ???? this is a stone! sentense,Look at
	 * this!
	 * 
	 * @param sentence  ???滻?????
	 * @param oStart    ??????
	...
	public static boolean isID(String str) {
		return !isEmpty(str.trim()) && str.trim().length() == 32
				|| NumberHelper.string2Int(StringHelper.null2String(str.trim())) != -1 || (str.indexOf("_") >= 0);// 598000024_B42394B
																													// 解决物料编码和调色号xx_xx作为浏览框唯一标识存储的情况
	}

	public static boolean isIDwithout_(String str) {
		return !isEmpty(str.trim()) && str.trim().length() == 32
	...
	// add by huairuo_zheng 20210719
	public static String formatMutis(String ids) {
		String ret = "";
		ArrayList arrayids = string2ArrayList(ids, ",");
		for (int i = 0; i < arrayids.size(); i++) {
			String _id = null2String(String.valueOf(arrayids.get(i))).trim();
	...
	public static String toHtml(String s, boolean f) {
		...
		while (i < c.length) {
			...
			// 以下注释 TD2432 关于字段中含有单引号的问题 董平 2005-11-11 (光棍节 呵呵~),重新加入，处理插入数据插入
			// by ben 2005-12-29
			if (ch == '<')
				buf.append("&lt;");
			else if (ch == '>')
